...
public class CrearReservaApp extends JPanel {
	...
	public CrearReservaApp(NavegadorApp nav, GestorDatos gestor) {
		this.nav = nav;
		this.gestor=gestor;
		...
		TButton reservar = new TButton("RESERVAR", () -> {
			...
			try {
				this.gestor.crearReserva(categoria.getText(), fechaRecogida, ubicacionR.getText(),
						ubicacionE.getText(), rangoEntrega, (Cliente) gestor.getUsuarioActual(), null);
