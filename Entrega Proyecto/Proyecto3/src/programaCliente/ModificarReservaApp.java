...
public class ModificarReservaApp extends JPanel {
	...
	public ModificarReservaApp(NavegadorApp nav, GestorDatos gestor) {
		this.nav = nav;
		this.gestor=gestor;

		final List<Reserva> reservas = gestor.getReservas();
		...
		this.reservar = new TButton("RESERVAR", () -> {
			...
			try {
				this.gestor.modificarReserva(reservaSeleccionada.getId(), fechaRecogida, rangoEntrega);
				this.nav.paginaAnterior();
